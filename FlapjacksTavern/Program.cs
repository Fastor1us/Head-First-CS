namespace FlapjacksTavern
{
  internal class Program
  {
    ...
    static readonly Queue<Classes.Lumberjack> lumberjackQueue = new();
    ...
    static void MakeQueue()
    {
      do
      {
        ...
        Classes.Lumberjack lumberjack = new(name);
        for (int i = 0; i < numberOfFlapjacks; i++)
        {
          int flapjackLength = Enum.GetValues(typeof(Enums.Flapjack)).Length;
          lumberjack.TakeFlapjack((Enums.Flapjack)random.Next(flapjackLength));
        }
